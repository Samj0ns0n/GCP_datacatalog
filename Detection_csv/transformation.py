...
def transformation_dataframe(bucket_name, blob_name):
    ...
    with fs.open('test-datapod/Test_header.csv', 'rt') as csvfile:
        ...
        if has_header:
            dataframe = pd.read_csv(csvfile, header=0, sep=dial_sep, engine ='python', encoding = infer_encoding, quoting=csv.QUOTE_NONE)
        else:
            dataframe = pd.read_csv(csvfile, header=None, sep=dial_sep, engine ='python', encoding = infer_encoding, quoting=csv.QUOTE_NONE)
        csvfile.close()
